...
- [ ] **Frontend monorepo scaffolding** — `docs/trd/07-web-frontend-architecture.md` — pnpm workspaces, three Vite React apps (apps/parent, apps/admin, apps/monitor), tsconfig.base.json, shared ESLint + Prettier config, path aliases (@ararat/ui, @ararat/api-client, @ararat/shared)
...
- [ ] **Shared types & validation schemas (packages/shared)** — `docs/trd/07-web-frontend-architecture.md` — Zod validation schemas (member, attendance, payment, simsa, auth, notification, newsletter, class, settings) shared between frontend and backend, TypeScript types/interfaces, constants (enums, belt levels, roles, error codes)
